...
function Counter () {
  ...
  const getPercent = () => {
    if (secondsRemaining && secondsAll && secondsRemaining / secondsAll > 0) {
  ...
  useInterval(
    () => {
      if (secondsRemaining > 0) {
        setSecondsRemaining(secondsRemaining - 0.1)
        setSecondsElapsed(secondsElapsed + 0.1)
      } else {
  ...
  return (
    <Grommet>
      <Main pad='large'>
        <div className='App'>
          <HeaderInfo />
          <DiagramPercentage getPercent={getPercent} />
          <DisplayStatus status={status} getPercent={getPercent} />
